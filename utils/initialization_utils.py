...
def is_bnb_available():
    """
    Check if bitsandbytes is installed and can be imported.
        ...
        if target_module_found:
            ...
            if reconstruction_mode == 'separated':
                if key in named_grads:
                    weight_key = key
                elif key + ".weight" in named_grads:
                    weight_key = key + ".weight"
                elif key.replace('q.pro', 'q_proj.weight') in named_grads:
                    weight_key = key.replace('q.pro', 'q_proj.weight')
                else:
                    print(f"Warning: Cannot find gradient for key: {key}")
                    continue 
                
                replacement_encoder_weight, replacement_decoder_weight, Sr = get_replacement_module_grad(
                    weight=named_grads[weight_key].T,
                    type=reconstr_type,     
